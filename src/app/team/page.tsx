...
async function load(slug: string): Promise<Display | null> {
  ...
  if (ready) {
    try {
      ...
      if (data) {
        // Merge rich static fields (stats/expertise/etc.). Match by slug first,
        // then by NAME — so a DB member (uuid slug) still gets their rich detail.
        const staticM =
          getMember(slug) ||
          TEAM.find(
            (t) => t.full_name.trim().toLowerCase() === data.full_name.trim().toLowerCase()
          );
        return {
          slug,
          full_name: data.full_name, title: data.title, level: data.level, bio: data.bio,
          photo_url: data.photo_url || staticM?.photo_url || "/logo-small.png", email: data.email || undefined,
